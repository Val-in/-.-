...
TaskCounts AS (
    SELECT AssignedTo, COUNT(*) AS TotalTasks
    FROM Tasks
    GROUP BY AssignedTo
)
SELECT 
...
LEFT JOIN Projects p ON s.DepartmentID = p.DepartmentID
LEFT JOIN Tasks t ON s.EmployeeID = t.AssignedTo
LEFT JOIN TaskCounts tc ON s.EmployeeID = tc.AssignedTo
LEFT JOIN DirectSubordinates ds ON s.EmployeeID = ds.ManagerID
GROUP BY s.EmployeeID, s.EmployeeName, s.ManagerID, d.DepartmentName, r.RoleName, tc.TotalTasks, ds.TotalSubordinates
ORDER BY s.EmployeeName;
...
WITH RECURSIVE AllSubordinates AS (
    ...
    FROM Employees e
    INNER JOIN AllSubordinates s ON e.ManagerID = s.EmployeeID
),
...
WHERE r.RoleName = 'Менеджер' AND COALESCE(sc.TotalSubordinates, 0) > 0
